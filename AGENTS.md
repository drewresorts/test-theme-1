...
- Splash **email signup** (Shopify newsletter) submits via AJAX with `Shopify.captcha.protect` so the page does not redirect to `/challenge`; the form must include `data-shopify-captcha="true"`. If sign-up is still blocked, disable hCaptcha under Online store → Preferences → Spam protection, or use the block’s **Custom HTML** / **External link** signup method.
